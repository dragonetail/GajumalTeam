...
class DetailPhotosViewController: UIViewController {
    ...
    @IBOutlet var trushButton: UIBarButtonItem!
    ...
    let dummy : [String : Any] = ["aaa" : (Any).self]
...
//    }
    ...
    func updateToolbars() {
        ...
        playButton.isEnabled = asset.canPerform(.content) && asset.playbackStyle != .imageAnimated
        ...
        if assetCollection != nil {
            trushButton.isEnabled = assetCollection.canPerform(.removeContent)
        } else {
            trushButton.isEnabled = asset.canPerform(.delete)
        }
        
        // Set the appropriate toolbarItems based on the playbackStyle of the asset.
        if asset.playbackStyle == .video {
            #if os(iOS)
                toolbarItems = [favoriteButton, space, playButton, space, trushButton]
                ...
                toolbarItems = [favoriteButton, space, trushButton]
                navigationController?.isToolbarHidden = false
            #elseif os(tvOS)
    ...
    func applyPhotoFilter(_ filterName: String, input: PHContentEditingInput, output: PHContentEditingOutput, completion: () -> Void) {
        ...
        let outputImage = inputImage
            ...
            .applyingFilter(filterName, parameters: dummy)
    ...
    func applyLivePhotoFilter(_ filterName: String, input: PHContentEditingInput, output: PHContentEditingOutput, completion: @escaping () -> Void) {
        ...
        livePhotoContext.frameProcessor = { frame, _ in
            return frame.image.applyingFilter(filterName, parameters: self.dummy)
        }
    ...
    func applyVideoFilter(_ filterName: String, input: PHContentEditingInput, output: PHContentEditingOutput, completion: @escaping () -> Void) {
        ...
        let composition = AVVideoComposition(
            ...
            applyingCIFiltersWithHandler: { request in
                let filtered = request.sourceImage.applyingFilter(filterName, parameters: self.dummy)
...
extension DetailPhotosViewController: PHPhotoLibraryChangeObserver {
...
extension DetailPhotosViewController: PHLivePhotoViewDelegate {
